...
export function ItemAestheticPanel({ item }: { item: WardrobeItem }) {
  ...
  if (loading) return <div className="mt-8 hairline-t pt-6 text-sm text-graphite/60"><Loader2 className="mr-2 inline h-4 w-4 animate-spin" />正在读取这件衣服的设计信息…</div>;

  return (
    <section id="item-aesthetic-panel" className="mt-6 scroll-mt-20 hairline-t pt-5" aria-labelledby="item-aesthetic-title">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {!analysis && (
            <button type="button" disabled={working || !item.imageUrl} onClick={() => void runAnalysis()} className="inline-flex min-h-10 items-center gap-2 bg-ink px-3.5 text-sm text-white disabled:opacity-45">
      ...
      {!consented && !analysis && <p className="mt-3 max-w-xl border-l-2 border-stamp/50 pl-2 text-xs leading-5 text-stamp">发送图片前需要你的同意；不会发送故事、评分或完整衣柜。</p>}
      {notice && <p className="mt-4 border-l-2 border-ink/70 pl-3 text-sm leading-6 text-ink/80">{notice}</p>}
      ...
      {analysis && !editing && hasFields && (
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <div className="border border-graphite/15 bg-tag/70 p-4">
            <div className="flex items-center justify-between gap-3"><h3 className="text-sm font-medium">结构与风格</h3><span className={`font-tag text-[10px] uppercase tracking-[0.15em] ${confirmed ? 'text-ink/70' : 'text-stamp'}`}>{confirmed ? '已确认' : '待确认'}</span></div>
          ...
          <div className="border border-graphite/15 bg-tag/70 p-4">
            <h3 className="font-tag text-[10px] uppercase tracking-[0.2em] text-graphite/60">单品色卡</h3>
            <div className="mt-4 flex flex-wrap items-end gap-4">{draft.dominantColors.map((color, index) => {
              const rgb = { r: color.rgb[0], g: color.rgb[1], b: color.rgb[2] };
              return <ColorSwatchCard key={`${color.hex}:${index}`} color={rgb} role={ROLE_LABELS[color.role]} size={color.role === 'dominant' ? 'lg' : 'sm'} />;
            })}</div>
          </div>
        </div>
      )}
      ...
      {analysis && editing && (
        <div className="mt-5 border border-graphite/20 bg-tag/75 p-4 sm:p-5">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            ...
            <button type="button" disabled={working || !item.imageUrl} onClick={() => void runAnalysis()} className="inline-flex min-h-11 items-center gap-2 border border-graphite/25 bg-white px-4 text-sm disabled:opacity-45">
              {working ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}重新解析
            </button>
            <button type="button" onClick={() => { setDraft(clonePayload(analysis?.payload)); setEditing(false); setError(''); }} className="min-h-11 px-4 text-sm text-graphite hover:text-ink">取消</button>
          </div>
        </div>
      )}
    </section>
