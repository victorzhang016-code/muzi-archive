...
function OutfitDetail({ outfit, run, onRefresh }: { outfit: OutfitCaseGraph; run: AnalysisRun; onRefresh: () => void }) {
  ...
  return <div className="border-t border-dashed border-graphite/20 p-4 sm:p-5">
    ...
    <div className="mt-4">
      ...
      {meaningful.length ? <div className="mt-2 grid gap-2 md:grid-cols-3">{meaningful.map((decision) => <div key={decision.id} className={`border p-3 ${decision.role === 'dominant' ? 'border-stamp/40 bg-stamp/5' : 'border-graphite/15 bg-white/55'}`}><p className="text-xs text-stamp">{OPERATION_LABELS[decision.operation] || decision.operation}</p><p className="mt-2 text-sm leading-6">{humanizeDisplayText(decision.action)}</p><p className="mt-2 text-xs leading-5 text-graphite/65">结果：{humanizeDisplayText(decision.effect)}</p></div>)}</div> : <p className="mt-2 border border-dashed border-graphite/25 p-3 text-sm leading-6 text-graphite/65">当前还不能从现有服装信息确认具体处理。原始搭配说明仍被保留。</p>}
    </div>
...
export function AestheticProfilePage() {
  ...
  return <section className="mx-auto max-w-6xl pb-14">
    <header className="grid gap-5 border-b border-dashed border-graphite/25 pb-6 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-end">
      <div><p className="font-tag text-[10px] uppercase tracking-[0.24em] text-stamp">Aesthetic profile</p><h1 className="mt-2 font-story text-4xl font-semibold leading-tight">你的穿衣方法</h1><p className="mt-3 max-w-2xl text-sm leading-7 text-graphite/75">从你确认过的单品信息、Best Match、变体和文字说明里整理。每条规律都能回到具体搭配。</p></div>
      <dl className="grid grid-cols-3 border border-graphite/20 bg-tag/65 text-center"><div className="p-3"><dt className="text-[10px] text-graphite/50">已确认单品</dt><dd className="mt-1 text-xl font-semibold">{confirmedCount}</dd></div><div className="border-x border-graphite/15 p-3"><dt className="text-[10px] text-graphite/50">Best Match</dt><dd className="mt-1 text-xl font-semibold">{run.outfitCases.length}</dd></div><div className="p-3"><dt className="text-[10px] text-graphite/50">可追溯规律</dt><dd className="mt-1 text-xl font-semibold">{run.principles.length}</dd></div></dl>
    </header>
