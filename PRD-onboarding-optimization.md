...
历史代码确认存在 Google 和邮箱登录/注册，但曾经没有可靠阻断未验证账号、保存作者来源，也没有降低首件录入成本。当前实现已在 Auth.tsx 中检查 email_confirmed_at，支持重发注册验证邮件和 `/auth/confirm` 回调；公开作者页通过 OnboardingIntent 保存来源与安全 next；空衣柜提供照片可选的 Quick Add，名称 + 品类即可保存，完整表单仍保留为备用入口。剩余验收项是实际 SMTP 投递、邮件链接回跳白名单及线上多路径手动测试。
