...
- 2026-07-21 追加修正：若三按钮在任一端出现竖排，不能只调整列数；必须在实际 DOM 上锁定单行 `flex` 轨道、允许子项收缩，并分别验收移动端与 Web 端，确认没有旧的 `grid` 或 `flex-wrap` 规则覆盖。
- 2026-07-21 结构根因修复：此前批量操作容器缺少关闭标签，使“整柜公开”和“添加衣物”错误嵌套在其内部，导致两端都纵向堆叠。现已重建为三个直接兄弟按钮；后续 UI 验收先检查 DOM 兄弟关系，再检查视觉规则。
- 2026-07-21 桌面端尺寸收口：三按钮的移动端等分宽度不再继承到 Web 端；桌面端恢复内容驱动的自然宽度，移动端保留三等分单行。生产验收已在 Victor 的真实登录 Chrome 标签完成：桌面端容器高度 `48px`、按钮宽度约 `135/121/127px`；`390px` 移动端容器高度 `44px`、三按钮同一条基线。验收前先刷新 SPA 页面，避免旧会话保留旧 DOM。
...
- 桌面端导航改为左侧固定栏：挂绳 + 三张吊牌（衣橱 / Best Match / 我的穿衣规律），激活页吊牌着墨落下；移动端新增底部 Tab 栏（穿孔+麻绳激活指示），顶栏只留 logo 与头像菜单。
- 全站装饰性虚线改为两端渐细 1px 实线（hairline-taper）；上传区/空槽保留功能性虚线。
- Archive：整柜公开不再重复出现；卡片操作触屏常显、统一 `.icon-action` 尺寸 token；移动端品类筛选横向滚动；删除隐藏死代码若干。
- 详情页：桌面端 55/45 分栏（左列完整大吊牌限宽 430px sticky）；移除整页点击返回；分享降为白底红字描边；删除增加二次确认；水洗标可读性修复（10px + textPrimary）；评分圈移动端单行。
